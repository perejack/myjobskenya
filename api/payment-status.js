...
async function queryMpesaPaymentStatus(checkoutId) {
  ...
  try {
    const response = await fetch(mpesaProxyUrl, {
      ...
      body: JSON.stringify({
        checkoutId,
        ...(mpesaProxyApiKey ? { apiKey: mpesaProxyApiKey } : {}),
      }),
    });
...
export default async (req, res) => {
  ...
  try {
    ...
    const supabase = getSupabaseClient(); // optional
    const mpesaProxyUrl = getMpesaProxyUrl();

    if (!mpesaProxyUrl) {
      return res.status(500).json({
        success: false,
        message: "Server misconfigured: missing credentials (MPESA_PROXY_URL:missing)",
      });
    }
    ...
    // If a DB exists, prefer it (faster + allows status caching/updating). Otherwise fall back to proxy-only.
    if (supabase) {
      ...
      if (dbError) {
        ...
        if (paymentStatus === "PENDING" && attempt.checkout_request_id) {
          ...
          if (proxyResponse && proxyStatus === "SUCCESS") {
            await supabase.from("payment_attempts").update({ status: "success" }).eq("id", attempt.id);
            paymentStatus = "SUCCESS";
    ...
    // No DB record found (or DB missing). Ask proxy directly.
