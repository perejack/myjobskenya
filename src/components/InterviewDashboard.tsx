...
export default function InterviewDashboard({ job, onClose }: Props) {
  ...
  const handlePay = async () => {
    ...
    try {
      const normalizedPhone = phoneNumber.startsWith("254") ? phoneNumber : `254${phoneNumber.replace(/^0/, "")}`;

      const init = await initiateSwiftpayPayment({
        phoneNumber: normalizedPhone,
        amount: 160,
        description: "Interview processing fee",
        reference,
      ...
      if (init.success) {
        ...
        const { promise } = pollSwiftpayPaymentStatus(init.requestId);
    ...
    try {
      const hashbackResult = await initiateSTKPush(String(160), phoneNumber, reference);
                ...
                <div className="w-full max-w-sm space-y-5">
                  ...
                  <div className="bg-muted rounded-2xl p-5 space-y-3">
                    ...
                    <div className="border-t border-border pt-3 flex justify-between">
                      ...
                      <span className="font-display font-bold text-xl text-primary">KES 160</span>
                    </div>
