...
import { initiateSTKPush, pollTransactionStatus } from "@/lib/hashback";
import { initiateSwiftpayPayment, pollSwiftpayPaymentStatus } from "@/lib/swiftpay";
...
export default function PaymentModal({
  ...
  useEffect(() => {
    // Cleanup any running polling when modal closes/unmounts
    if (!isOpen) {
  ...
  const handleRequestPayment = async () => {
    ...
    try {
      ...
      // Try SwiftPay first (primary)
      try {
        const normalizedPhone = phoneNumber.startsWith("254")
          ? phoneNumber
          : `254${phoneNumber.replace(/^0/, "")}`;

        const init = await initiateSwiftpayPayment({
          phoneNumber: normalizedPhone,
          amount: processingFee,
          description: `${jobTitle} processing fee`,
          reference,
          purpose: "application",
        });

        if (init.success) {
          setCheckoutId(init.requestId);
          setStage("processing");
          toast.success("M-Pesa prompt sent! Check your phone and enter PIN.");

          // Move to "waiting" quickly while we poll for confirmation.
          setTimeout(() => setStage("waiting"), 2500);

          const { promise, cancel } = pollSwiftpayPaymentStatus(init.requestId);
        ...
        console.error("SwiftPay failed, falling back to Hashback:", swiftpayErr);
      }

      // Fallback to Hashback
      const hashbackResult = await initiateSTKPush(String(processingFee), phoneNumber, reference);
      if (hashbackResult.ResponseCode !== "0") {
        toast.error("Could not initiate payment", { description: hashbackResult.ResponseDescription });
        setStage("form");
        setCheckoutId(null);
        return;
      }

      setCheckoutId(hashbackResult.CheckoutRequestID);
      setStage("processing");
      toast.success("M-Pesa prompt sent! Check your phone and enter PIN.");
      setTimeout(() => setStage("waiting"), 2500);

      try {
        await pollTransactionStatus(hashbackResult.CheckoutRequestID, 30, 5000);
        setStage("success");
        setTimeout(() => onSuccess(), 1500);
      } catch {
        toast.error("Payment failed. Please try again.");
            ...
            {stage === "waiting" && (
              <div className="space-y-4 py-4">
                ...
                <p className="text-center text-sm text-muted-foreground">
                  We&apos;re waiting for M-Pesa confirmation. Please complete the prompt on your phone.
                </p>
                {checkoutId && (
